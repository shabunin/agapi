...
| 2026-09-03 | **New gallery group: Examples** — composed recipes spanning multiple `agapi.*` modules, distinct from single-module labs. First entry: **WebRTC P2P (LAN)** (`WebrtcP2PTool.tsx`) — fully serverless peer-to-peer over the local network. `RTCPeerConnection` stays a plain browser global (never wrapped in stdlib, same rule as the existing WebRTC browser-probe); agapi supplies only the rendezvous — `agapi.mdns` for discovery (`_agapi-webrtc._tcp`) and `agapi.http.WebSocketServer`/`WebSocket` for a one-time local SDP/ICE signaling exchange (trickle ICE both ways). `iceServers: []` — no STUN/TURN, nothing leaves the LAN at any point. Not a new stdlib API, just a recipe built entirely from already-shipped primitives |
